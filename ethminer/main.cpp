...
#undef BACKTRACE_MAX_FRAMES
    ...
    void help()
    {
        cout << "Ethminer - GPU ethash miner" << endl
             << "minimal usage : etcminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default etcminer will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes etcminer can stay" << endl
                 ...
                 << "    --exit              FLAG Stop etcminer whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how etcminer chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects etcminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    etcminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'etcminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When etcminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode etcminer tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    cout << "etcminer " << bi->project_version << endl
         << "Build:   " << bi->system_name << "/" << bi->build_type << "/" << bi->compiler_id << endl
	 << "Source:  https://github.com/obykk/etcminer" << endl
         ;

    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'etcminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try etcminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
